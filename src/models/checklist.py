...
class ChecklistStatusEnum(Enum):
    OK = "OK"
    ERROR = "ERROR"
    LOADING = "LOADING"
...
checklist_example = [
    {
        ...
        "5": "Функциональный код разбит на модули: модуль для взаимодействия с API, модуль для взаимодействия с файлами, модуль для взаимодействия с вакансиями ",
...
class Checklist(BaseModel):
    ...
    motivation: str = Field(default="", description="Мотивашка после выполнения", examples=["Ты молодец, ты выполнил задание"])

    body: list[dict] = Field(default=[], description="Тело чеклиста", examples=[checklist_example])
    status: ChecklistStatusEnum = Field(default=ChecklistStatusEnum.LOADING, description="Статус чеклиста", examples=["OK", "ERROR", "LOADING"])
